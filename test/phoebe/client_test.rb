...
class PhoebeTest < Minitest::Test
  include WebMock::API
  ...
  def test_client_default_request_default_retry_attempts
    stub_request(:get, "http://localhost/ref/hotspot/info/locId").to_return_json(status: 500, body: {})

    phoebe = Phoebe::Client.new(base_url: "http://localhost", api_key: "My API Key")

    assert_raises(Phoebe::Errors::InternalServerError) do
      phoebe.ref.hotspot.info.retrieve("locId")
    end
  ...
  def test_client_given_request_default_retry_attempts
    ...
    phoebe = Phoebe::Client.new(base_url: "http://localhost", api_key: "My API Key", max_retries: 3)
  ...
  def test_client_default_request_given_retry_attempts
    ...
    assert_raises(Phoebe::Errors::InternalServerError) do
      phoebe.ref.hotspot.info.retrieve("locId", request_options: {max_retries: 3})
    end
  ...
  def test_client_given_request_given_retry_attempts
    ...
    assert_raises(Phoebe::Errors::InternalServerError) do
      phoebe.ref.hotspot.info.retrieve("locId", request_options: {max_retries: 4})
    end
  ...
  def test_client_retry_after_seconds
    stub_request(:get, "http://localhost/ref/hotspot/info/locId").to_return_json(
      status: 500,
      headers: {"retry-after" => "1.3"},
      body: {}
    )

    phoebe = Phoebe::Client.new(base_url: "http://localhost", api_key: "My API Key", max_retries: 1)
  ...
  def test_client_redirect_307
    ...
    assert_raises(Phoebe::Errors::APIConnectionError) do
      phoebe.ref.hotspot.info.retrieve("locId", request_options: {extra_headers: {}})
    end

    recorded, = WebMock::RequestRegistry.instance.requested_signatures.hash.first

    assert_requested(:any, "http://localhost/redirected", times: Phoebe::Client::MAX_REDIRECTS) do
  ...
  def test_client_redirect_303
    ...
    assert_requested(:get, "http://localhost/redirected", times: Phoebe::Client::MAX_REDIRECTS) do
  ...
  def test_client_redirect_auth_strip_cross_origin
    ...
    assert_requested(:any, "https://example.com/redirected", times: Phoebe::Client::MAX_REDIRECTS) do
      headers = _1.headers.keys.map(&:downcase)
  ...
  def test_default_headers
    stub_request(:get, "http://localhost/ref/hotspot/info/locId").to_return_json(status: 200, body: {})
