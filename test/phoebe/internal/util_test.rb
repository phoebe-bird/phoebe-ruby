...
class Phoebe::Test::UtilDataHandlingTest < Minitest::Test
  def test_left_map
    assert_pattern do
      Phoebe::Internal::Util.deep_merge({a: 1}, nil) => nil
    end
  end

  def test_right_map
    assert_pattern do
      Phoebe::Internal::Util.deep_merge(nil, {a: 1}) => {a: 1}
    end
  end

  def test_disjoint_maps
    assert_pattern do
      Phoebe::Internal::Util.deep_merge({b: 2}, {a: 1}) => {a: 1, b: 2}
    end
  end

  def test_overlapping_maps
    assert_pattern do
      Phoebe::Internal::Util.deep_merge({b: 2, c: 3}, {a: 1, c: 4}) => {a: 1, b: 2, c: 4}
    end
  end

  def test_nested
    assert_pattern do
      Phoebe::Internal::Util.deep_merge({b: {b2: 1}}, {b: {b2: 2}}) => {b: {b2: 2}}
    end
  end

  def test_nested_left_map
    assert_pattern do
      Phoebe::Internal::Util.deep_merge({b: {b2: 1}}, {b: 6}) => {b: 6}
    end
  end

  def test_omission
    merged = Phoebe::Internal::Util.deep_merge(
      {b: {b2: 1, b3: {c: 4, d: 5}}},
      {b: {b2: 1, b3: {c: Phoebe::Internal::OMIT, d: 5}}}
    )
  ...
  def test_dig
    assert_pattern do
      Phoebe::Internal::Util.dig(1, nil) => 1
      Phoebe::Internal::Util.dig({a: 1}, :b) => nil
      Phoebe::Internal::Util.dig({a: 1}, :a) => 1
      Phoebe::Internal::Util.dig({a: {b: 1}}, [:a, :b]) => 1

      Phoebe::Internal::Util.dig([], 1) => nil
      Phoebe::Internal::Util.dig([nil, [nil, 1]], [1, 1]) => 1
      Phoebe::Internal::Util.dig({a: [nil, 1]}, [:a, 1]) => 1
      Phoebe::Internal::Util.dig([], 1.0) => nil

      Phoebe::Internal::Util.dig(Object, 1) => nil
      Phoebe::Internal::Util.dig([], 1.0) { 2 } => 2
      Phoebe::Internal::Util.dig([], ->(_) { 2 }) => 2
      Phoebe::Internal::Util.dig([1], -> { _1 in [1] }) => true
    end
  end
end

class Phoebe::Test::UtilUriHandlingTest < Minitest::Test
  def test_parsing
    %w[
      http://example.com
      https://example.com/
      ...
      parsed = Phoebe::Internal::Util.parse_uri(url)
      unparsed = Phoebe::Internal::Util.unparse_uri(parsed).to_s

      assert_equal(url, unparsed)
      assert_equal(parsed, Phoebe::Internal::Util.parse_uri(unparsed))
    end
  end
  ...
  def test_joining
    cases = [
      [
        "h://a.b/c?d=e",
        "h://nope/ignored",
        Phoebe::Internal::Util.parse_uri("h://a.b/c?d=e")
      ],
      [
        "h://a.b/c?d=e",
        "h://nope",
    ...
    cases.each do |expect, lhs, rhs|
      assert_equal(
        URI.parse(expect),
        Phoebe::Internal::Util.join_parsed_uri(
          Phoebe::Internal::Util.parse_uri(lhs),
          rhs
        )
      )
    end
  ...
  def test_joining_queries
    ...
    cases.each do |path, expected|
      assert_equal(
        URI.parse(expected),
        Phoebe::Internal::Util.join_parsed_uri(
          Phoebe::Internal::Util.parse_uri(base_url),
          {path: path}
        )
      )
    end
...
class Phoebe::Test::RegexMatchTest < Minitest::Test
  def test_json_content
    ...
    cases.each do |header, verdict|
      assert_pattern do
        Phoebe::Internal::Util::JSON_CONTENT.match?(header) => ^verdict
      end
    end
  end
  ...
  def test_jsonl_content
    ...
    cases.each do |header, verdict|
      assert_pattern do
        Phoebe::Internal::Util::JSONL_CONTENT.match?(header) => ^verdict
      end
    end
  end
end

class Phoebe::Test::UtilFormDataEncodingTest < Minitest::Test
  class FakeCGI < CGI
    def initialize(headers, io)
      encoded = io.to_a
      @ctype = headers["content-type"]
      # rubocop:disable Lint/EmptyBlock
      @io = Phoebe::Internal::Util::ReadIOAdapter.new(encoded.to_enum) {}
...
class Phoebe::Test::UtilIOAdapterTest < Minitest::Test
  def test_copy_read
    cases = {
      StringIO.new("abc") => "abc",
    ...
    cases.each do |input, expected|
      io = StringIO.new
      # rubocop:disable Lint/EmptyBlock
      adapter = Phoebe::Internal::Util::ReadIOAdapter.new(input) {}
      # rubocop:enable Lint/EmptyBlock
  ...
  def test_copy_write
    ...
    cases.each do |input, expected|
      enum = Phoebe::Internal::Util.writable_enum do |y|
...
class Phoebe::Test::UtilFusedEnumTest < Minitest::Test
  ...
  def test_thread_interrupts
    ...
    fused_1 = Phoebe::Internal::Util.fused_enum(enum, external: true) { loop { enum.next } }
    fused_2 = Phoebe::Internal::Util.chain_fused(fused_1) { fused_1.each(&_1) }
    fused_3 = Phoebe::Internal::Util.chain_fused(fused_2) { fused_2.each(&_1) }
  ...
  def test_closing
    arr = [1, 2, 3]
    once = 0
    fused = Phoebe::Internal::Util.fused_enum(arr.to_enum) do
      once = once.succ
    end
  ...
  def test_rewind_chain
    once = 0
    fused = Phoebe::Internal::Util.fused_enum([1, 2, 3].to_enum) do
      once = once.succ
    end
      .lazy
      .map(&:succ)
  ...
  def test_closed_fused_extern_iteration
    ...
    fused = Phoebe::Internal::Util.fused_enum(enum)
    first = fused.next
  ...
  def test_close_fused_sse_chain
    ...
    fused_1 = Phoebe::Internal::Util.fused_enum(enum)
    fused_2 = Phoebe::Internal::Util.decode_lines(fused_1)
    fused_3 = Phoebe::Internal::Util.decode_sse(fused_2)

    assert_equal(0, taken)
    Phoebe::Internal::Util.close_fused!(fused_3)
    assert_equal(0, taken)
  end
end

class Phoebe::Test::UtilContentDecodingTest < Minitest::Test
  def test_charset
    cases = {
    ...
    cases.each do |content_type, encoding|
      Phoebe::Internal::Util.force_charset!(content_type, text: text)
      assert_equal(encoding, text.encoding)
    end
  end
end

class Phoebe::Test::UtilSseTest < Minitest::Test
  def test_decode_lines
    ...
    cases.each do |enum, expected|
      eols.each do |eol|
        lines = Phoebe::Internal::Util.decode_lines(enum.map { _1.gsub("\n", eol) })
  ...
  def test_mixed_decode_lines
    ...
    cases.each do |enum, expected|
      lines = Phoebe::Internal::Util.decode_lines(enum)
  ...
  def test_decode_sse
    ...
    cases.each do |name, test_cases|
      test_cases.each do |input, expected|
        actual = Phoebe::Internal::Util.decode_sse(input).map(&:compact)
